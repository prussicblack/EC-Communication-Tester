...
EXP int CALL soem_config_init(int use_map)
{
   ...
   // v2.0: ecx_config_init(context) �� ���� 1��
   if (ret <= 0)
      return -1;
...
typedef struct soem_slave_info_t
{
   ...
   uint16 configadr;          // Station Address (���� �ּ�)
   uint32 vendor;             // eep_man
   uint32 product;            // eep_id
   uint32 revision;           // revision
   char name[EC_MAXNAME + 1]; // �����̺� �̸�(ESI)
} soem_slave_info_t;
...
{
   int32_t error_code; // ec_errort.ErrorCode (SDO abort, AL status ��)
   uint16_t slave;     // ���� �����̺� ��ȣ
   uint16_t index;     // CoE Index
   uint8_t subindex;   // CoE SubIndex
   uint8_t _pad;       // 1����Ʈ �е��� ����(���� ����)
   uint16_t etype;     // �Ʒ� 3)���� ���� (������ �ξ� ����)
} soem_error_info_t;
...
EXP int CALL soem_get_last_error_info(soem_error_info_t *info)
{
   ...
   info->_pad = 0;
   info->etype = (uint16_t)err.Etype; // �� �߰�
   return 1;
}
